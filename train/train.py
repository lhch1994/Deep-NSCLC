...
import torchvision.models as models_inpytorch
...
def validation(val_loader, FCCM_net, epoch, train_writer=None):
    ...
    for i, (CT_img, ground_truth) in enumerate(tqdm(val_loader)):
        with torch.no_grad():
            ...
            predict_classification = torch.sigmoid(predict_result).cpu().detach().numpy()
            ...
            Clinical_N_stage = np.argmax(predict_classification[:, 0:5],1)
            Histology = np.argmax(predict_classification[:, 5:10],1)
            Overall_Stage = np.argmax(predict_classification[:, 10:14],1)
            Clinical_T_stage = np.argmax(predict_classification[:, 14:19],1)

            GT_Clinical_N_stage = np.argmax(ground_truth[:, 0:5],1)
            GT_Histology = np.argmax(ground_truth[:, 5:10],1)
            GT_Overall_Stage = np.argmax(ground_truth[:, 10:14],1)
            GT_Clinical_T_stage = np.argmax(ground_truth[:, 14:19],1)
